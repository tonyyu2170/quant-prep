...
describe("problem registry invariants", () => {
  ...
  it("every non-choice template is playable in the market game", () => {
    ...
    expect(MARKET_TEMPLATES.length).toBeGreaterThanOrEqual(256);
  });
  ...
  it("exact-count problems grade strictly from their own tolerance object", () => {
    ...
    expect(exact.length).toBe(31);   // 17 counting exact counts, 5 choice templates, 7 exact brainteaser answers, 2 statistics sample sizes
