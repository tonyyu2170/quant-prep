...
export const gbmExpectedPrice: ProblemTemplate = {
  ...
  solution: (p, d) => [
    { title: "The average log is not the log of the average", body: `A drift quoted on the log price fixes the MEDIAN outcome, because the log is symmetric about its own mean. The price itself is that log exponentiated, and exponentiating is convex — so the average price sits above the median price, by an amount that grows with the volatility and nothing else. Writing $h$ for that half-variance, $\\text{mean}=\\text{median}\\times e^{h}$.` },
