...
export const atLeastOneComplement: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Every roll missing", body: `The rolls are independent, so the chance that every one of the ${fmtNum(p.rolls)} rolls misses is that fraction multiplied by itself once per roll: $\\left(\\frac{${fmtNum(d.missFaces)}}{${fmtNum(p.faces)}}\\right)^{${fmtNum(p.rolls)}}=${fmtNum(d.allMiss)}$. Everything else is the event we want, so the answer is $1-\\left(\\frac{${fmtNum(d.missFaces)}}{${fmtNum(p.faces)}}\\right)^{${fmtNum(p.rolls)}}=${fmtNum(d.prob)}$.` },
