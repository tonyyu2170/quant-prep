...
export const allOneTypeDraw: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Sanity check", body: `Price a different experiment: the chef takes one avocado, looks at it, drops it back, and repeats. Each look is ripe with probability $${fmtNum(p.ripe)}/${fmtNum(d.total)}=${fmtNum(d.ripeShare)}$, so that experiment would answer $${fmtNum(d.ripeShare)}^{${fmtNum(p.grab)}}=${fmtNum(d.withRepl)}$. In the real crate nothing goes back, so each ripe avocado already taken leaves the crate slightly less ripe and every later pick is harder — the true answer must land strictly under the replacement figure, and $${fmtNum(d.prob)} < ${fmtNum(d.withRepl)}$.` },
  ],
