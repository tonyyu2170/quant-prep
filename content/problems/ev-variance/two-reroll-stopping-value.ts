...
export const twoRerollStoppingValue: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Value the first spin", body: `Now the bar is higher, because throwing the first spin away no longer buys a plain spin — it buys the second-spin game, which carries a throw of its own. So the first spin is kept only when it beats ${fmtNum(d.midValue)} points, and anything below ${fmtNum(d.topKeep)} goes back. Those ${fmtNum(d.topLow)} sectors are worth ${fmtNum(d.midValue)} points each and ${
      d.topLow === p.sectors - 1
        ? `the ${fmtNum(d.topTopSum)} points on the one sector you keep stand`
        : `the ${fmtNum(d.topTopSum)} points on the sectors you keep stand`
