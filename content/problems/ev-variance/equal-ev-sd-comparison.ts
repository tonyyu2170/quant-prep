...
export const equalEvSdComparison: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Spread of the die", body: `The die's squared payout averages $\\frac{${fmtNum(p.k)}\\times${fmtNum(d.prize)}\\times${fmtNum(d.prize)}}{${fmtNum(p.faces)}}=${fmtNum(d.meanSqDie)}$, and taking off the square of the mean — the same ${fmtNum(d.varCoin)} as the coin's variance, since the two games average the same amount — leaves a variance of $${fmtNum(d.meanSqDie)}-${fmtNum(d.varCoin)}=${fmtNum(d.varDie)}$ squared dollars. Back in dollars that is $\\sqrt{${fmtNum(d.varDie)}}=${fmtNum(d.sdDie)}$.` },
    { title: "Sanity check", body: `Rebuild the die's variance straight from the definition instead — each outcome's distance from the mean, squared and weighted by the faces it happens on: $\\frac{${fmtNum(p.k)}\\times(${fmtNum(d.prize)}-${fmtNum(p.m)})\\times(${fmtNum(d.prize)}-${fmtNum(p.m)})+${fmtNum(d.blankFaces)}\\times${fmtNum(p.m)}\\times${fmtNum(p.m)}}{${fmtNum(p.faces)}}=${fmtNum(d.varDie)}$ squared dollars, matching. And the die had to come out the riskier of the two, though not because all of its outcomes are more extreme: a face paying nothing sits ${fmtNum(p.m)} dollars below the mean, exactly where the coin's tails sits. What moves is the paying side, which the die pushes out to ${fmtNum(d.prize)} dollars against the coin's ${fmtNum(d.coinPay)} — and because the variance adds up squared distances, that one far outcome dominates the rest.` },
  ],
  keyInsight: "Two bets with the same expected payout can carry completely different risk, because the mean is blind to how the payout is spread across outcomes. Concentrating the same average onto fewer, larger payoffs leaves most outcomes exactly where they were — paying nothing is the same disappointment in either game — and throws the paying one far out; since variance adds up squared distances, that single outlier is enough to dominate the spread.",
