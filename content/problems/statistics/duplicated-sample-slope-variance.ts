...
// Duplicating a sample doubles the spread of X about its mean and so halves the slope's
// variance — the classic "it looks like twice the data" error, which is really the same data
// counted twice. The error variance is stated as KNOWN rather than estimated: an estimated one
...
export const duplicatedSampleSlopeVariance: ProblemTemplate = {
  ...
  statement: (p) =>
    ...
    `A colleague then appends a second copy of the whole data set, so every observation appears twice. What is the variance of the fitted slope on the doubled data?`,
  solution: (p, d) => [
    ...
    { title: "Sanity check", body: `The fitted slope itself does not move at all: every residual is duplicated too, so the same line still minimises the sum of squares. What changed is only the reported variance, which now claims the precision of ${fmtNum(d.rowsNew)} independent observations while the data still carry the information of ${fmtNum(p.n)}. That is the error — the arithmetic is right and the independence assumption it rests on is false.` },
  ],
