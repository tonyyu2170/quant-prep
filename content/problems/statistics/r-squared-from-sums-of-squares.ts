...
export const rSquaredFromSumsOfSquares: ProblemTemplate = {
  ...
  constraint: (p) => p.rss >= 0.08 * p.tss && p.rss <= 0.92 * p.tss,
  derived: (p) => {
  ...
  statement: (p) =>
    `A strategy's daily P&L is regressed on a single risk factor over a long sample. Measured about its own mean the P&L has a total sum of squares of ${fmtNum(p.tss)}, and the fitted regression leaves a residual sum of squares of ${fmtNum(p.rss)}. ` +
  ...
  solution: (p, d) => [
    ...
    { title: "The total variation splits in two, and only two", body: `Write $T$ for the total sum of squares of the response about its own mean, $U$ for the part the fit leaves unexplained — the residual sum of squares — and $E$ for the part it explains. Least squares makes that split exact, $T=E+U$, with nothing left over. The share of the variation the fit accounts for is therefore the explained part over the whole, $R^{2}=\\dfrac{E}{T}=1-\\dfrac{U}{T}$, and the two forms are the same number reached from opposite ends.` },
  ...
  keyInsight: "R-squared is a ratio of sums of squares, not a measure of whether the fit is right: it says what share of the response's variation the fitted line accounts for in this sample, and nothing about whether the relationship is causal, stable, or even linear. Adding any predictor at all can only push it up, which is why it cannot be used to choose between models of different size.",
