...
export const overlappingWindowSums: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Sanity check", body: `Count it the other way as a check. X plus Y is the same as adding up every day in either window, with the shared days counted twice — so it is a weighted sum of ${fmtNum(p.a)} plus ${fmtNum(p.b)} independent day terms in which ${fmtNum(p.ov)} of them carry a weight of two. Squaring the weights gives the same total, because a day at weight two contributes four times one day's variance rather than twice.` },
  ],
