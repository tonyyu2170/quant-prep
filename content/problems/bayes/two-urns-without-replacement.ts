...
export const twoUrnsWithoutReplacement: ProblemTemplate = {
  ...
  derived: (p) => {
    ...
    return { aTotal, bTotal, pTwoRedA, pTwoRedB, massA, massB, denom, postA };
  },
  statement: (p) =>
  ...
  solution: (p, d) => [
    ...
    { title: "Combine", body: `$P(A,RR)=0.5\\times${fmtNum(d.pTwoRedA)}=${fmtNum(d.massA)}$, $P(B,RR)=0.5\\times${fmtNum(d.pTwoRedB)}=${fmtNum(d.massB)}$. $P(RR)=${fmtNum(d.massA)}+${fmtNum(d.massB)}=${fmtNum(d.denom)}$.` },
    { title: "Posterior", body: `$P(A\\mid RR)=${fmtNum(d.massA)}/${fmtNum(d.denom)}=${fmtNum(d.postA)}$.` },
