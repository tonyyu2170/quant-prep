...
export const bookmakerOddsUpdate: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Prior odds", body: `The shared ${fmtNum(d.denomOdds)} underneath both cancels: $\\text{odds}(\\text{win})=P(\\text{win})/P(\\text{lose})=1/${p.againstOdds}=${fmtNum(d.priorOdds)}$ — this recovers the bookmaker's original $1:${p.againstOdds}$ quote.` },
    { title: "Update with the likelihood ratio", body: `$\\text{odds}(\\text{win}\\mid\\text{signal})=\\text{odds}(\\text{win})\\times\\text{LR}=\\frac{1}{${p.againstOdds}}\\times${p.likelihoodRatio}=${fmtNum(d.postOdds)}$.` },
    { title: "Convert back to a probability", body: `Odds of $x$ mean probability $x/(x+1)$; multiplying top and bottom by ${p.againstOdds} clears the fraction. $P(\\text{win}\\mid\\text{signal})=\\dfrac{${p.likelihoodRatio}}{${p.likelihoodRatio}+${p.againstOdds}}=${fmtNum(d.postProb)}$.` },
