...
export const creditBureauDisagreement: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Combine", body: `Risky mass: $${p.prior}\\times${fmtNum(d.riskyLikelihood)}=${fmtNum(d.riskyMass)}$. Safe mass: $${fmtNum(d.safe)}\\times${fmtNum(d.safeLikelihood)}=${fmtNum(d.safeMass)}$. So $P(A,\\bar B)=${p.prior}\\times${fmtNum(d.riskyLikelihood)}+${fmtNum(d.safe)}\\times${fmtNum(d.safeLikelihood)}=${fmtNum(d.denom)}$.` },
    { title: "Posterior", body: `$P(R\\mid A,\\bar B)=\\dfrac{${p.prior}\\times${fmtNum(d.riskyLikelihood)}}{${p.prior}\\times${fmtNum(d.riskyLikelihood)}+${fmtNum(d.safe)}\\times${fmtNum(d.safeLikelihood)}}=${fmtNum(d.posterior)}$.` },
