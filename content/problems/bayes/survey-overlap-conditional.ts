...
export const surveyOverlapConditional: ProblemTemplate = {
  ...
  statement: (p) =>
    `A survey finds that ${p.countB} respondents identify as morning people. Of all respondents surveyed, ${p.countAB} are both morning people and daily coffee drinkers. ` +
    `Picking a morning-person respondent at random, what is the probability they are also a daily coffee drinker?`,
  answerKey: "pAgivenB",
  accepted: { tolerance: { rel: 0.005 } },
  solution: (p, d) => [
    { title: "Setup", body: `Let $A$ = daily coffee drinker, $B$ = morning person. Given the joint count $${p.countAB}$ and the marginal count $${p.countB}$.` },
    { title: "Apply the definition", body: `$P(A\\mid B) = \\dfrac{\\text{count}(A\\cap B)}{\\text{count}(B)}$.` },
