...
export const spamFilterOdds: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Posterior odds", body: `$\\text{odds}(S\\mid H) = \\text{odds}(S)\\times\\text{LR} = \\frac{${p.priorSpam}}{${fmtNum(d.priorHam)}}\\times\\frac{${p.pPhraseSpam}}{${p.pPhraseHam}} = ${fmtNum(d.posteriorOdds)}$.` },
    { title: "Convert to probability", body: `Odds of $x$ mean probability $x/(x+1)$. Clearing the two denominators leaves the two branch weights themselves: $P(S\\mid H) = \\dfrac{${p.priorSpam}\\times${p.pPhraseSpam}}{${p.priorSpam}\\times${p.pPhraseSpam}+${fmtNum(d.priorHam)}\\times${p.pPhraseHam}} = ${fmtNum(d.posterior)}$.` },
