...
export const prosecutorsFallacyMatch: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Combine", body: `Both branches share the common factor $1/${fmtNum(p.pop)}$, so in relative units $P(G,M)\\propto1$ and $P(\\bar G,M)\\propto${fmtNum(d.others)}\\times${p.q}=${fmtNum(d.falseMatchMass)}$ — giving $P(M)\\propto1+${fmtNum(d.others)}\\times${p.q}=${fmtNum(d.denom)}$.` },
    { title: "Posterior", body: `$P(G\\mid M)=\\dfrac{1}{1+${fmtNum(d.others)}\\times${p.q}}=${fmtNum(d.postGuilty)}$, so $P(\\text{innocent}\\mid M)=\\dfrac{${fmtNum(d.others)}\\times${p.q}}{1+${fmtNum(d.others)}\\times${p.q}}=${fmtNum(d.postInnocent)}$.` },
