...
export const networkOutageJointAlerts: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Posterior", body: `$P(S_1\\cap S_2)=${p.prior}\\times${fmtNum(d.jointD)}+${fmtNum(d.legit)}\\times${p.jointND}=${fmtNum(d.denom)}$, so $P(D\\mid S_1\\cap S_2)=\\dfrac{${p.prior}\\times${fmtNum(d.jointD)}}{${p.prior}\\times${fmtNum(d.jointD)}+${fmtNum(d.legit)}\\times${p.jointND}}=${fmtNum(d.postD)}$.` },
