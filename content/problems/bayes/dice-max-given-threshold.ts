...
export const diceMaxGivenThreshold: ProblemTemplate = {
  ...
  solution: (p, d) => {
    ...
    return [
      ...
      { title: "Enumerate the favorable pairs", body: `Pairs whose MAXIMUM equals exactly ${p.m} come from two cases: the first die is ${p.m} and the second is anywhere from one up to ${p.m} (${p.m} pairs), or the second die is ${p.m} and the first is anywhere from one up to ${p.m} (${p.m} pairs) — but the double roll $(${p.m},${p.m})$ is counted in both, so subtract one: $${p.m}+${p.m}-1=${fmtNum(d.favorable)}$ pairs total: ${favorableList}.` },
  ...
  commonTrap: "Dividing the favorable count by the full unconditioned sample space of every ordered pair, instead of by the smaller total that the conditioning event leaves — the conditioning event always rules out at least one pair, so its total is always smaller than dividing by every possible roll would give.",
  expectedPaceS: 90,
