...
export const twoSignalFraud: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Update on Alert B", body: `Treat $\\frac{${fmtNum(d.numA)}}{${fmtNum(d.denomA)}}$ as the new prior. True-fire mass: $\\frac{${fmtNum(d.numA)}}{${fmtNum(d.denomA)}}\\times${p.sensB}=${fmtNum(d.numB)}$. False-fire mass: the legitimate share is now $\\frac{${fmtNum(d.fpA)}}{${fmtNum(d.denomA)}}$, giving $\\frac{${fmtNum(d.fpA)}}{${fmtNum(d.denomA)}}\\times${p.fprB}=${fmtNum(d.fpB)}$. That shared denominator ${fmtNum(d.denomA)} cancels out of the ratio, so $P(F\\mid A,B)=\\dfrac{${fmtNum(d.numA)}\\times${p.sensB}}{${fmtNum(d.numA)}\\times${p.sensB}+${fmtNum(d.fpA)}\\times${p.fprB}}=${fmtNum(d.post2)}$.` },
