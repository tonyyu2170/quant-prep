...
export const twoSignalFraud: ProblemTemplate = {
  ...
  statement: (p) =>
    `A trading platform flags transactions using two independent alert systems. The prior probability any transaction is fraudulent is ${pc(p.prior)}%. ` +
  ...
  keyInsight: "Independent evidence compounds sequentially: feed each posterior back in as the next prior instead of recombining both signals from scratch.",
