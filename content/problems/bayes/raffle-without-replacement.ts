...
export const raffleWithoutReplacement: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Joint branches and total", body: `$P(F,S)=${fmtNum(d.pFirstWin)}\\times${fmtNum(d.pSecondLossGivenFirstWin)}=${fmtNum(d.jointWinThenLoss)}$ and $P(\\bar F,S)=${fmtNum(d.pFirstLoss)}\\times${fmtNum(d.pSecondLossGivenFirstLoss)}=${fmtNum(d.jointLossThenLoss)}$. So $P(S)=${fmtNum(d.jointWinThenLoss)}+${fmtNum(d.jointLossThenLoss)}=${fmtNum(d.pSecondLoss)}$.` },
    { title: "Bayes", body: `$P(F\\mid S)=${fmtNum(d.jointWinThenLoss)}/${fmtNum(d.pSecondLoss)}=${fmtNum(d.postFirstWin)}$.` },
