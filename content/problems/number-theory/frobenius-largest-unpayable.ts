...
export const frobeniusLargestUnpayable: ProblemTemplate = {
  ...
  solution: (p, d) => [
    ...
    { title: "Why the two remaining leave anything out", body: `The two share no factor above one. If they did, every reachable total would be a multiple of that factor and infinitely many totals would be unreachable — there would be no largest. Sharing nothing is exactly what makes the answer finite. For such a pair the largest gap is $\\text{largest}=ab-a-b$.` },
